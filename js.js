...
var IS_SETUP_COMPLETE = false; // Flag to hold audio frames until setup completes
...
async function startMic() {
  ...
  WORKLET_NODE.port.onmessage = (EVENT) => {
    // DO NOT send audio frames until WebSocket is OPEN AND setup is complete
    if (!IS_TALKING || !IS_SETUP_COMPLETE || !SOCKET || SOCKET.readyState !== WebSocket.OPEN) return;
...
function handleServerMessage(RAW_DATA) {
  ...
  // 1. Wait for setupComplete before sending greeting or sending audio
  if (MESSAGE.setupComplete) {
    console.log("Setup complete. Initiating teacher session...");
